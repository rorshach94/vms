...
def home(request):
    ...
    context["dapartments"] = (
        models.Departments.objects.filter(delete_flag=0, status=1).all().count()
    )
    ...
    day = date.strftime("%d")
    context["visitors"] = (
        models.Visitors.objects.filter(
            date_added__year=year, date_added__month=month, date_added__day=day
        )
        .all()
        .count()
    )
